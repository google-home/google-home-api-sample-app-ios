...
public class AutomationList: ObservableObject {
  ...
  @Published public var automationsUIModels = [AutomationUIDataModel]()
  @Published public var automations = [any Automation]()
  ...
  public func add(_ automation: any Automation) {
    ...
    automationsUIModels.append(AutomationUIDataModel(draftAutomation: automation))
  }
    ...
    do {
      ...
      if let index = automations.firstIndex(where: {
        ...
        await MainActor.run {
          ...
          automationsUIModels[index] = AutomationUIDataModel(draftAutomation: updatedAutomation)
        }
      } else {
        Logger().error(
    ...
    async throws
  {
    do {
      ...
      if let index = automations.firstIndex(where: {
        ...
        automationsUIModels.remove(at: index)
