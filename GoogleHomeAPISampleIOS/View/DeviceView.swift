...
struct DeviceView: View {
  ...
  @State private var enteredDeviceName: String = ""
  ...
  var body: some View {
    ...
    .alert("Device Rename", isPresented: self.$isShowingNameInput) {
      TextField("Device Name", text: self.$enteredDeviceName)
      Button("Rename", role: .none) {
        self.renameDeviceTo(name: self.enteredDeviceName)
      }
    ...
    .contextMenu {
      ...
      Button("Rename...", systemImage: "rectangle.and.pencil.and.ellipsis") {
        self.enteredDeviceName = self.deviceControl.device.name
        self.isShowingNameInput = true
      }
    }
  }
  
  private func renameDeviceTo(name: String) {
    Task {
      do {
        try await self.deviceControl.device.setName(name)
      } catch {
    ...
    List {
      ...
      ForEach(entries) { entry in
        Button(entry.roomName) {
          ...
          Task {
            do {
              try await self.structure.move(device: self.deviceControl.device.id, to: entry.roomID)
            } catch {
