  ...
  var body: some View {
          ...
          .toolbar {
            if mainViewModel.structures.count > 0 {
              ToolbarItemGroup(placement: .principal) {
                ...
                let sortedStructures = self.mainViewModel.structures.sorted { $0.name < $1.name }
                let selectedID = self.mainViewModel.selectedStructureID
                let selectedStructure = self.mainViewModel.structure(structureID: selectedID)
                Menu(selectedStructure?.name ?? "Select Structure") {
                  ForEach(sortedStructures) { structure in
                    let isSelected = structure.id == selectedID
                    Button(structure.name, systemImage: isSelected ? "checkmark" : "house") {
    ...
    var conditionalStructureView: some View {
      let structureViewModel =
        self.mainViewModel.structureViewModel(structureID: self.mainViewModel.selectedStructureID)
